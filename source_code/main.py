...
result_folder = 'D:/ANPR-main/result_data'

# Lấy mô hình YOLO
license_plate_detector = YOLO('D:/ANPR-main/model/last.pt')

# Khởi tạo EasyOCR reader
reader = easyocr.Reader(['en'])

# Lấy danh sách các tên file và các biến cap phục vụ cho việc đuọc và lưu dứ liệu
list_cap, list_filename = read_files_in_directory('D:/ANPR-main/input_data/video')
...
for id in range(len(list_cap)):
    ...
    while list_cap[id].isOpened():
        ...
        for i, box in enumerate(license_plate_boxes):
            ...
            plate_filename = f'{directory_path}\Licenseplate_{frame_id}_{i}.jpg' #lưu ảnh bsx vào thư mục bên trên
            license_plate.save(plate_filename) # hàm lưu
            print(f'Luu vao {plate_filename}') #
            ...
            if results:
                data = {"Video": i+1, "Duong dan": plate_filename, "Thoi gian": frame_id//fps, 'Bien so': results[0][1]}
                print("Data:",data)
                save_data.append(data)
                print(f"Khung hinh {frame_id} - Bien so tu video {i + 1} la: {results[0][1]}")
